...
function setPreguntasDisponibles(){
    const totalPreguntas = quiz.length;
    for(let i = 0; i<totalPreguntas; i++){
        preguntasDisponibles.push(quiz[i])
    }
}
...
function obtenerNuevaPregunta(){
    ...
    preguntasNumero.innerHTML = "Pregunta " + (preguntaContador + 1) + " de " + quiz.length;
    //colocar pregunta
    ...
    preguntasTexto.innerHTML = preguntaActual.q;
    ...
    for(let i = 0; i<opcionLongitud; i++){
        ...
        //obtener la posicio de opcionIndex desde la matriz opcionesDispo
...
function getResult(element){
    ...
    if(id === preguntaActual.answer){
        ...
        for(let i=0; i<opcionLongitud; i++){
            if(parseInt(contenedorOpcion.children[i].id) === preguntaActual.answer){
...
function siguiente(){
    if(preguntaContador === quiz.length){
...
function quizResultado(){
    ...
    const porcentaje = (respuestasCorrectas/quiz.length) * 100;
